...
test('the TV breathes before assembling and its side screens settle on Broadcast stills', () => {
  ...
  assert.match(television, /AUTO_REVEAL_DELAY_MS\s*=\s*8000/)
  assert.match(television, /AUTO_REVEAL_DURATION\s*=\s*14/)
  ...
  assert.match(television, /a0 = 0\.34 \+ i \* 0\.065/)
  assert.match(television, /travel = lerp\(0\.20, 0\.36, inertia\)/)
