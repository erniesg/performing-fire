...
rucksack github token erniesg/performing-fire --role developer --execute >/dev/null
rucksack github token erniesg/performing-fire --role publisher --execute >/dev/null
```

Install or update the held timer and credential-gated daily CLI updater from the repository root:

```bash
rucksack vm autopilot install-timer erniesg/performing-fire --repo-root . --profile trusted-worker --execute
```

Activate only this repository drain after the live isolation probes pass:

```bash
rucksack vm autopilot install-timer erniesg/performing-fire --repo-root . --profile trusted-worker --enable-drain --isolation bubblewrap --execute
```
...
```bash
rucksack vm autopilot discord erniesg/performing-fire --profile trusted-worker --execute
```
...
cp infra/vm/systemd/rucksack-autopilot-v1-ZXJuaWVzZy9wZXJmb3JtaW5nLWZpcmU-drain.service ~/.config/systemd/user/
cp infra/vm/systemd/rucksack-autopilot-v1-ZXJuaWVzZy9wZXJmb3JtaW5nLWZpcmU-drain.timer ~/.config/systemd/user/
loginctl enable-linger "$USER"
systemctl --user daemon-reload
systemctl --user disable --now rucksack-autopilot-v1-ZXJuaWVzZy9wZXJmb3JtaW5nLWZpcmU-drain.timer
systemctl --user status rucksack-autopilot-v1-ZXJuaWVzZy9wZXJmb3JtaW5nLWZpcmU-drain.timer
```
...
```bash
journalctl --user -u rucksack-autopilot-v1-ZXJuaWVzZy9wZXJmb3JtaW5nLWZpcmU-drain.service -f
```

Manual equivalent:

```bash
rucksack autopilot review-repair erniesg/performing-fire --provider vm-codex --execute
rucksack autopilot self-heal erniesg/performing-fire --repo-root . --provider vm-codex --request-review codex --execute
rucksack autopilot work-queue erniesg/performing-fire --provider vm-codex --max-workers 1 --local --repo-root . --issue-dir docs/issues --reconcile-issues --check-provider-ready --notify-github-when-blocked --execute
```
...
```bash
rucksack autopilot work erniesg/performing-fire --issue ISSUE_NUMBER --provider vm-codex --execute
```
