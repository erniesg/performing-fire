...
This directory stores reviewable markdown issue specs for `erniesg/performing-fire`.

Generate or update specs with:

```bash
rucksack github issues plan erniesg/performing-fire --repo-root . --issue-dir docs/issues --execute
```
...
```bash
rucksack github issues seed erniesg/performing-fire --issue-dir docs/issues --label rucksack-ledger --execute
```
...
```bash
gh issue reopen ISSUE_NUMBER --repo erniesg/performing-fire
```
...
```bash
gh workflow enable rucksack-autopilot.yml --repo erniesg/performing-fire
gh variable set RUCKSACK_AUTOPILOT_ENABLED --repo erniesg/performing-fire --body true
rucksack autopilot reconcile erniesg/performing-fire --issue-dir docs/issues --queue-after-activation --execute
gh workflow run rucksack-autopilot.yml --repo erniesg/performing-fire -f action=queue
```
...
`rucksack autopilot resolve erniesg/performing-fire --issue 123 --decision accept --repo-root <checkout> --run-post-unlock --execute`
instead of dispatching cloud work immediately.
When review evidence is accepted, use `/rucksack done #123` or
`rucksack autopilot resolve erniesg/performing-fire --issue 123 --decision done --execute` to
...
```bash
rucksack autopilot next erniesg/performing-fire --repo-root . --issue-dir docs/issues --provider vm-codex
```
...
```bash
rucksack autopilot submit erniesg/performing-fire --issue 123 --repo-root . --execute
```
